...
active_bCREs=$projectDir/active_bCREs
...
bcres=/zata/zippy/phanh/Registry/V4/GRCh38/bCREs_all.bed
...
awk -v FS='\t' -v OFS='\t' 'NR==FNR{a[$1] = $2 FS $3; next} ($4 in a) {print $0,a[$4]; next}' \
	$cell_type.rDHSs_filtered.txt bCREs_all.bed > $cell_type.bCREs.bed
...
cp $cell_type.bCREs.bed $projectDir/$2
